...
async function handleGroupManagement(rl) {
  while (true) {
    const currentGroups = await groupService.getAllGroups();
    const currentMode = groupService.getCurrentMode();

    const option = await showGroupManagementMenu(
      rl,
      currentGroups,
      currentMode
    );
...
async function handleAddGroup(rl) {
  ...
  const newGroup = await groupService.addGroup(link, isFirstGroup);
...
async function handleEditGroup(rl) {
  ...
  const success = await groupService.updateGroup(id, newLink);
  if (success) {
...
async function handleSetPrimaryGroup(rl) {
  ...
  groups.forEach((group, index) => {
    console.log(
      `${index + 1}. ${group.id} | ${group.link} ${
        group.isPrimary ? "(ATUAL)" : ""
      }`
    );
  });
  ...
  await groupService.setPrimaryGroup(selectedGroup.id);
