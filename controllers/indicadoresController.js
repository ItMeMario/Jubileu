...
function processCompleteStatistics(stats) {
    ...
    if (stats.horariosEscolhidos) {
        ...
        if (totalHorarios > 0) {
            const horariosProcessados = Object.entries(stats.horariosEscolhidos).map(([id, horario]) => {
            ...
            if (horariosComEscolhas.length > 1) {
                horarioMenosPopular = horariosComEscolhas
                    .reduce((prev, current) => prev[1].count < current[1].count ? prev : current);
            }
...
function processHourlyStatistics(hourlyStats) {
    ...
    if (totalHorarios === 0) {
        const horariosDisponiveis = Object.entries(hourlyStats).map(([id, horario]) => ({
            id,
            horario: horario.horario
        }));
