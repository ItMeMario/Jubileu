...
function showCurrentScoutConfig(scoutConfig) {
  ...
  if (scoutConfig.lastChanged) {
    console.log(
      `Última alteração: ${new Date(scoutConfig.lastChanged).toLocaleString(
        "pt-BR"
      )}`
    );
  } else {
...
function showDetailedStatus(status) {
  ...
  console.log(
    `Modo: ${status.isDevMode ? "🔧 DESENVOLVIMENTO" : "🚀 PRODUÇÃO"}`
  );
  console.log(
    `Debug: ${status.debugEnabled ? "🐛 HABILITADO" : "🔇 DESABILITADO"}`
  );
  ...
  console.log(
    `Última alteração (Modo): ${status.lastChanged || "Não registrado"}`
  );
  console.log(
    `Última alteração (Debug): ${status.lastDebugChanged || "Não registrado"}`
  );
  console.log(
    `Última alteração (Scout): ${status.lastScoutChanged || "Não registrado"}`
  );
  console.log(
    `Arquivo de config: ${
      status.configExists ? "✅ Encontrado" : "❌ Não encontrado"
    }`
  );
  console.log("\n" + "=".repeat(40));
}
...
function showCurrentLocale(currentLocale) {
  console.log(`\n🌐 Idioma atual: ${currentLocale}`);
}

function showAvailableLocales(availableLocales) {
  console.log("\n🌐 IDIOMAS DISPONÍVEIS");
  console.log("=".repeat(25));

  availableLocales.forEach((locale) => {
    console.log(`${locale.index}. ${locale.code}`);
  });

  console.log("=".repeat(25));
}

function showLocaleChangeSuccess(newLocale) {
  console.log("✅ Idioma alterado com sucesso!");
  console.log(`🌐 Novo idioma: ${newLocale}`);
  console.log("📝 As mensagens do sistema agora usarão este idioma!");
}

function showLocaleMenu(currentLocale, availableLocales) {
  console.log("\n🌐 CONFIGURAÇÃO DE IDIOMA");
  console.log("=".repeat(30));
  console.log(`Idioma atual: ${currentLocale}`);
  console.log("\nIdiomas disponíveis:");

  availableLocales.forEach((locale) => {
    const isCurrent = locale.code === currentLocale;
    const marker = isCurrent ? " (atual)" : "";
    console.log(`${locale.index}. ${locale.code}${marker}`);
  });
